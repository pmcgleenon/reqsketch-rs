...
mod capacity_calculation_tests {
    ...
    fn test_capacity_calculation_correctness() {
        ...
        assert_eq!(
            level0_capacity, 72,
            "Level 0 capacity should be 72, got {}",
            level0_capacity
        );
    }
    ...
    fn test_item_density_consistency() {
        ...
        let total_weighted_items: u64 = level_info
            .iter()
            .map(|(_, items, _, weight)| *items as u64 * weight)
            .sum();

        // Validate weight conservation
        assert_eq!(
            total_weighted_items, stream_size as u64,
    ...
    fn test_level_structure_matches_cpp() {
        ...
        assert!(
            (4..=7).contains(&num_levels),
            "Expected 4-7 levels, got {}",
            num_levels
        );

        assert!(
            (301..500).contains(&total_retained),
            "Expected ~300-500 retained, got {}",
            total_retained
        );
    }
}
...
mod global_capacity_tests {
    ...
    fn test_global_capacity_constraint() {
        ...
        for i in 0..10000 {
            ...
            assert!(
                total_retained <= total_capacity + 50,
    ...
    fn test_individual_level_capacity_management() {
        ...
        for (level, items, capacity, _weight) in sketch.level_info() {
            if items > 0 {
                ...
                assert!(
                    utilization <= 3.0,
                    "Level {} exceeds reasonable bounds: {}x",
                    level,
                    utilization
                );
            }
        ...
        assert!(
            total_capacity > 0,
            "Total nominal capacity must be positive"
        );

        // Global utilization should be reasonable (allow some overhead for C++ strategy)
        assert!(
            global_utilization <= 1.5,
            "Global utilization too high: {}x",
            global_utilization
        );
    }
...
mod error_bounds_validation_tests {
    ...
    fn test_error_bounds_accuracy_across_k_values() {
        ...
        for &k in &k_values {
            ...
            assert!(
                ...
                "Should be in estimation mode for k={}",
                k
            );
            ...
            assert!(
                error_bound <= expected_bound * 4.0, // Allow more generous bound
    ...
    fn test_hra_vs_lra_accuracy_guarantees() {
        ...
        for &rank in &test_ranks {
            ...
            if rank >= 0.75 {
                assert!(
                    hra_error <= lra_error * 1.1, // Allow 10% tolerance
                ...
                assert!(
                    lra_error <= hra_error * 1.1, // Allow 10% tolerance
...
mod internal_state_consistency_tests {
    ...
    fn test_sorted_items_remain_sorted() {
        ...
        for item in sorted_view.iter() {
            if let Some(prev) = prev_value {
                assert!(
                    item >= prev,
                    "Items not properly sorted: {} came after {}",
                    item,
                    prev
                );
            }
    ...
    fn test_merge_preserves_algorithmic_properties() {
        ...
        assert!(
            total_capacity > 0,
            "Total nominal capacity after merge must be positive"
        );
        ...
        assert!(
            total_retained <= total_capacity * 3, // Much higher tolerance for C++ strategy
...
mod cross_validation_tests {
    ...
    fn test_rank_calculation_consistency() {
        ...
        for &rank in &test_ranks {
            ...
            assert!(
                error <= theoretical_error * 2.0, // Allow 2x theoretical error for round-trip
    ...
    fn test_comparative_accuracy_with_theoretical_bounds() {
        ...
        for &rank in &test_ranks {
            ...
            assert!(
                error <= 0.1, // Allow 10% error tolerance for C++ strategy
