    ...
    for &rank in &test_ranks {
        ...
        for (sigma, &(lower, upper)) in bounds.iter().enumerate() {
            ...
            assert!(
                ...
                "{}-sigma lower bound out of [0,1] for rank {}: {}",
                sigma,
                rank,
                lower
            );
            assert!(
                ...
                "{}-sigma upper bound out of [0,1] for rank {}: {}",
                sigma,
                rank,
                upper
            );
        ...
        assert!(
            ...
            "2-sigma bounds should contain 1-sigma bounds for rank {}",
            rank
        );
        assert!(
            bounds[2].0 <= bounds[1].0 && bounds[1].1 <= bounds[2].1,
            "3-sigma bounds should contain 2-sigma bounds for rank {}",
            rank
        );
    }
}
